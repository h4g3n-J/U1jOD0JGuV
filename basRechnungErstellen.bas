...
Public Function RechnungErstellenCreateRecordset()
    ...
    ElseIf DCount("[EAkurzKey]", "tblEinzelauftrag", "[EAkurzKey] Like '" & varEAIDRechnung & "'") = 0 Then
        ...
        MsgBox "Bitte w‰hlen Sie im Feld 'Einzelauftrag (Rechnung)' ausschlieﬂlich Werte aus der Drop-Down-Liste.", vbCritical, "Speichern abgebrochen"
        Exit Function
    End If
    ...
    If DCount("[RechnungNr]", "tblRechnung", "[RechnungNr] Like '" & varRechnungNummer & "'") > 0 Then
        ...
        MsgBox "ƒnderungen gespeichert", vbOKOnly, "Speichern"
    Else
