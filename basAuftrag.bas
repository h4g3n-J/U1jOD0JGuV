...
Public Sub SelectRecordset(ByVal varRecordsetName As Variant)
    ...
    If gconVerbatim = True Then
        Debug.Print "Form_frmAuftragSuchenSub.SelectRecordset: varRecordsetName = " + CStr(varRecordsetName)
    End If
        
    Dim strAuftragName  As String
    strAuftragName = CStr(varRecordsetName)
    
    Set gobjAuftrag1 = New clsAuftrag
    gobjAuftrag1.SelectRecordset strAuftragName, bolVerbatim
    
    Forms.Item("frmSearchMain").Controls.Item("txt0") = gobjAuftrag1.AftrID
    ' Forms.Item("frmSearchMain").txt0 = gobjAuftrag1.AftrID
    
End Sub

Public Sub ShowRecordset(ByVal strRecordsetName As String)
    If gconVerbatim = True Then
        Debug.Print "basAuftrag.ShowRecordset: strRecordsetName = " & strRecordsetName
    End If
